...
export default function Post({ refreshTrigger }) {
  ...
  const fetchPosts = async (pageNum) => {
    ...
    try {
      const res = await fetch(
        `https://massoudnet-backend.onrender.com/api/posts/feed?page=${pageNum}`,
        {
          credentials: "include",
        }
      );
      ...
      setPosts((prev) => [
        ...prev,
        ...data.filter((p) => !prev.some((x) => x._id === p._id)),
      ]);
  ...
  // When refreshTrigger flips, fetch page 1 and prepend new posts
  useEffect(() => {
    if (typeof refreshTrigger === "undefined") return;

    const fetchLatest = async () => {
      try {
        const res = await fetch(
          `https://massoudnet-backend.onrender.com/api/posts/feed?page=1`,
          {
            credentials: "include",
          }
        );
        ...
        const data = await res.json();
        setPosts((prev) => [
          // only posts not already in state
          ...data.filter((p) => !prev.some((x) => x._id === p._id)),
          ...prev,
        ]);
      } catch {
        toast.error("Error loading new posts");
      }
    };
